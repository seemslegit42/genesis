'use client';

import dynamic from 'next/dynamic';
import { Skeleton } from '@/components/ui/skeleton';
import { Sidebar, SidebarProvider, SidebarInset } from '@/components/ui/sidebar';
import { Sidecar } from '@/components/sidecar';
import { Card, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';

const ChatSession = dynamic(
  () => import('@/components/chat-session').then((mod) => mod.ChatSession),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-screen">
        <Sidebar className="hidden md:block" collapsible="icon">
           <div className="p-4"><Skeleton className="h-8 w-full" /></div>
           <div className="flex-1 p-2 space-y-2">
            <Skeleton className="h-8 w-full" />
            <Skeleton className="h-8 w-full" />
            <Skeleton className="h-8 w-full" />
           </div>
        </Sidebar>
        <div className="flex-1 flex flex-col">
          <header className="fixed top-0 z-20 w-full glassmorphism h-[70px]">
            <div className="flex items-center justify-between p-4 h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 gap-4">
                <Skeleton className="h-6 w-24" />
                <Skeleton className="h-12 w-full max-w-2xl rounded-full" />
                <Skeleton className="h-8 w-40 hidden md:block" />
            </div>
          </header>
          <main className="flex-1 flex flex-col overflow-hidden pt-[70px]">
            <div className="flex flex-col items-center justify-center h-full p-4">
              <div className="flex-1 flex items-center justify-center w-full">
                <Skeleton className="w-24 h-80" />
              </div>
              <div className="pb-8 w-full max-w-4xl mx-auto">
                  <div className="flex flex-col items-center justify-center text-center">
                      <Skeleton className="h-8 w-64 mb-4" />
                      <Skeleton className="h-4 w-96 mb-8" />
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-3xl">
                          <Skeleton className="h-16 rounded-lg" />
                          <Skeleton className="h-16 rounded-lg" />
                          <Skeleton className="h-16 rounded-lg" />
                          <Skeleton className="h-16 rounded-lg" />
                      </div>
                  </div>
              </div>
            </div>
          </main>
        </div>
      </div>
    ),
  }
);

/**
 * The main entry point for the BEEP: Genesis application, the portal to the Initiate's Journey.
 * It establishes the three-column Command Cockpit layout, consisting of a persistent Sidebar for
 * high-level orchestration, the central ChatSession for direct command, and the Sidecar for
 * proactive guidance. This structure is designed to be a scalable and intuitive interface
 * for managing a complex underlying agentic system.
 * @returns {JSX.Element} The rendered chat page, the Initiate's cognitive sanctuary.
 */
export default function ChatPage() {
  return (
    <SidebarProvider>
        <div className="flex min-h-screen">
            <Sidebar className="hidden md:flex flex-col" collapsible="icon">
                {/* Placeholder for future agent/workflow navigation */}
            </Sidebar>
            <main className="flex-1 flex flex-col">
                <ChatSession />
            </main>
            <Sidecar className="hidden lg:flex w-full max-w-sm ml-6 flex-col">
                <Card className="bg-transparent border-0 h-full">
                    <CardHeader>
                        <CardTitle className='font-headline text-primary drop-shadow-[0_0_8px_hsl(var(--primary))]'>
                            Oracle's Whisper
                        </CardTitle>
                        <CardDescription>
                           Your proactive guide. Anticipating your next move.
                        </CardDescription>
                    </CardHeader>
                </Card>
            </Sidecar>
        </div>
    </SidebarProvider>
  );
}
